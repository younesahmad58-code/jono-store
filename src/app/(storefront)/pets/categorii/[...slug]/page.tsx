...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${category.name} | JONO Pets`,
    description: category.description ?? `Produse pets din categoria ${category.name}`,
  };
}

export default async function PetsCategoryPage({ params, searchParams }: Props) {
  const { slug } = await params;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
        ...
        {category.parent && (
          <>
            <a href={`/pets/categorii/${category.parent.slug}`} className="hover:text-foreground">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          ...
          <Pagination
            ...
            basePath={`/pets/categorii/${slug.join("/")}`}
          />
        </div>
      </div>
    </div>
